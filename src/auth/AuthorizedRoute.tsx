...
export const AuthorizedRoute = ({ children, loggedInUser, roles, all }:any) => {
  let authed = false;
  if (loggedInUser) {
    if (roles && roles.length) {
      authed = all
        ? roles.every((r:any) => loggedInUser.roles.includes(r))
        : roles.some((r:any) => loggedInUser.roles.includes(r));
    } else {
      authed = true;
    }
  }
